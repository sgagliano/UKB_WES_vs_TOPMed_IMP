#######################
## Compare the "true" (WES) genotypes in UK Biobank with imputed dosages (and genotypes) from TOPMed-imputated UK Biobank: SPLICE
## in R
...
out_fig      <- paste0(dir, "/splice.pearson.tiff")
out_fig0.3 <-paste0(dir, "/splice-impR20.3.pearson.tiff")
...
#get only splice
dat<-subset(dat, dat$splice_region_variant==1)
nrow(dat) #110857
...
#write out
write.table(res, "splice.csv", sep=",", row.names=F, quote=F)
...
#write out
write.table(res, "splice-impR20.3.csv", sep=",", row.names=F, quote=F)
