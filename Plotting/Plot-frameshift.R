#######################
## Compare the "true" (WES) genotypes in UK Biobank with imputed dosages (and genotypes) from TOPMed-imputated UK Biobank: FRAMESHIFT
## in R
...
out_fig      <- paste0(dir, "/frameshift.pearson.tiff")
out_fig0.3 <-paste0(dir, "/frameshift-impR20.3.pearson.tiff")
...
#get only frameshift
dat<-subset(dat, dat$frameshift_variant==1)
nrow(dat) #15962
...
#write out
write.table(res, "frameshift.csv", sep=",", row.names=F, quote=F)
...
#write out
write.table(res, "frameshift-impR20.3.csv", sep=",", row.names=F, quote=F)
