#######################
## Compare the "true" (WES) genotypes in UK Biobank with imputed dosages (and genotypes) from TOPMed-imputated UK Biobank: MISSENSE
## in R
...
out_fig      <- paste0(dir, "/missense.pearson.tiff")
out_fig0.3 <-paste0(dir, "/missense-impR20.3.pearson.tiff")
...
#get only missense
dat<-subset(dat, dat$missense_variant==1)
nrow(dat) #763591
...
#write out
write.table(res, "missense.csv", sep=",", row.names=F, quote=F)
...
#write out
write.table(res, "missense-impR20.3.csv", sep=",", row.names=F, quote=F)
