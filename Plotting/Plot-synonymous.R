#######################
## Compare the "true" (WES) genotypes in UK Biobank with imputed dosages (and genotypes) from TOPMed-imputated UK Biobank: SYNONYMOUS
## in R
...
out_fig      <- paste0(dir, "/synonymous.pearson.tiff")
out_fig0.3 <-paste0(dir, "/synonymous-impR20.3.pearson.tiff")
...
#get only synonymous
dat<-subset(dat, dat$synonymous_variant==1)
nrow(dat) #475997
...
#write out
write.table(res, "synonymous.csv", sep=",", row.names=F, quote=F)
...
#write out
write.table(res, "synonymous-impR20.3.csv", sep=",", row.names=F, quote=F)
